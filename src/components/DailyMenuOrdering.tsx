...
export default function DailyMenuOrdering({ studentId, onBack }: { studentId: number; onBack: () => void }) {
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full relative pb-24">
      {/* Header */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {vendors.map((vendor) => (
            <button
              ...
              className="glass-card group relative p-6 rounded-2xl text-left cursor-pointer transition-colors focus:outline-none overflow-hidden hover:border-zinc-500"
            >
              ...
              <h3 className="text-lg font-bold text-zinc-100">{vendor.name}</h3>
            </button>
          ))}
          {vendors.length === 0 && (
            ...
            <div key={item.id} className="glass-card p-4 rounded-xl flex items-center justify-between">
              ...
              <div className="flex items-center gap-3 bg-zinc-900 border border-zinc-800 rounded-lg p-1">
                <button 
                  ...
                  className="w-8 h-8 rounded-md bg-zinc-800 hover:bg-zinc-700 text-zinc-300 flex items-center justify-center transition-colors cursor-pointer disabled:opacity-50"
                ...
                <button 
                  onClick={() => handleUpdateCart(item, 1)}
                  className="w-8 h-8 rounded-md bg-zinc-800 hover:bg-zinc-700 text-zinc-300 flex items-center justify-center transition-colors cursor-pointer"
                >
      ...
      {view === 'menu' && cartItemCount > 0 && (
        <div className="fixed bottom-6 left-0 right-0 px-4 md:px-0 flex justify-center z-50 animate-fade-in pointer-events-none">
          <div className="w-full max-w-md bg-zinc-100 text-zinc-900 p-4 rounded-2xl shadow-2xl flex items-center justify-between pointer-events-auto">
            ...
            <button
              ...
              disabled={isLoading}
              className="px-6 py-3 bg-black text-white hover:bg-zinc-800 rounded-xl font-bold text-sm transition-colors shadow-md disabled:opacity-50 cursor-pointer"
            >
      ...
      {view === 'success' && orderResult && (
        <div className="glass-card p-12 rounded-2xl flex flex-col items-center text-center space-y-6">
          ...
          <div className="bg-zinc-950 border border-zinc-800 py-4 px-8 rounded-xl shadow-inner">
            <p className="text-3xl font-mono font-bold text-white tracking-widest">{orderResult.orderNumber}</p>
          </div>
          <p className="text-zinc-500 font-medium">Total Paid: ₹{orderResult.totalAmount}</p>
          <button
            onClick={onBack}
